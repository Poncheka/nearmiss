...
--
-- Only the month and day are ever shown, but the column is a date because that is what a date
-- picker gives you and what Postgres can validate. Optional, and blank for everyone until they
-- fill it in.
alter table public.profiles add column if not exists birthday date;

comment on column public.profiles.birthday is
  'Optional. Only the month and day are used, to mark near misses that fell on a birthday.';

-- my_near_misses gains the other person's birthday so the app can label the day without a
-- second round trip per row. Dropped first: Postgres will not change a function''s return type
-- in place.
